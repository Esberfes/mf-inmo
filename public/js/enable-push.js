        ...
        navigator.serviceWorker.getRegistrations().then(
            function(registrations) {
                for(let registration of registrations) { 
                    ...
                    versioncargada = registration.active.scriptURL.split('?')[1];
                    if (versioncargada != version) {
                        actualizar = true;
                        console.log('[PWA Builder] old active service worker found, updating...');
...
function registerSW() {
    navigator.serviceWorker.register('/pwabuilder-sw.js?' + version, {
		 scope: '/'
	  }).then(function(reg) {
